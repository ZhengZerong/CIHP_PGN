...
def parse_args():
    ...
    parser.add_argument('--in_dir', type=str,
                        required=True, help='path to image files')
...
def main(in_dir, out_dir):
    ...
    # create data list
    img_list = glob(os.path.join(in_dir, '*.jpg'))
    img_list += glob(os.path.join(in_dir, '*.png'))
    ...
    with tf.name_scope("create_inputs"):
        reader = ImageReader(in_dir, img_list_file,
                             None, None, False, False, False, coord)
    ...
    # evaluate prosessing
    parsing_dir = os.path.join(out_dir, 'cihp_parsing_maps')
    if not os.path.exists(parsing_dir):
        os.makedirs(parsing_dir)
    edge_dir = os.path.join(out_dir, 'cihp_edge_maps')
    if not os.path.exists(edge_dir):
        os.makedirs(edge_dir)
    # Iterate over training steps.
    for step in range(len(img_list)):
        ...
        fg_pred = np.uint8(parsing_ > 0).squeeze()
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5))
        fg_pred_erode = cv2.erode(fg_pred, kernel)
        fg_pred_dilate = cv2.dilate(fg_pred, kernel)
        mask = fg_pred_erode + 3 * (fg_pred_dilate - fg_pred_erode)

        bgdModel = np.zeros((1, 65), np.float64)
        fgdModel = np.zeros((1, 65), np.float64)
        img = cv2.imread(image_list[step])
        mask, bgdModel, fgdModel = cv2.grabCut(
            img, mask, None, bgdModel, fgdModel, 20, cv2.GC_INIT_WITH_MASK)
